...
<div class="page">
    ...
    <div class="page-header">
        <div>
            <div class="page-title">🥗 Aliments</div>
            <div class="page-sub">Gestion des aliments et nutriments</div>
        </div>
        <a href="/admin/aliments/create" class="btn btn-primary">+ Ajouter aliment</a>
    </div>
    ...
    <div class="card">
        <div class="card-head">
            <span class="card-title">Liste des aliments</span>
            <span style="font-size:11px;color:var(--muted);"><?= count($aliments) ?> aliments</span>
        </div>
        <div class="card-body">
                ...
                <table class="tbl">
                    ...
                    <tbody>
                        <?php foreach ($aliments as $aliment): ?>
                            <tr>
                                <td><strong><?= esc($aliment['nom']) ?></strong></td>
                                <td><?= number_format((float)($aliment['calories_100g'] ?? 0), 1) ?> kcal</td>
                                <td><?= number_format((float)($aliment['proteines_100g'] ?? 0), 1) ?> g</td>
                                <td><?= number_format((float)($aliment['glucides_100g'] ?? 0), 1) ?> g</td>
                                <td><?= number_format((float)($aliment['lipides_100g'] ?? 0), 1) ?> g</td>
                                <td>
                                    <div class="action-btns">
                                        <a href="/admin/aliments/edit/<?= $aliment['id'] ?>" class="btn btn-secondary btn-sm">Éditer</a>
                                        <form method="POST" action="/admin/aliments/delete/<?= $aliment['id'] ?>" style="display:inline;" onsubmit="return confirm('Supprimer cet aliment ?');">
                                            <?= csrf_field() ?>
            ...
            <?php else: ?>
                <div style="text-align:center;padding:40px;color:var(--muted);font-size:12px">
                    Aucun aliment trouvé. <a href="/admin/aliments/create" style="color:var(--accent);text-decoration:underline">Créer un aliment</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
