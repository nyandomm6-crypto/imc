...
<div class="page">
    ...
    <div class="breadcrumb">
        ...
        <span>/</span>
        <span><?= $sport ? 'Éditer' : 'Créer' ?></span>
    </div>

    <div class="page-title"><?= $sport ? 'Éditer sport' : 'Créer sport' ?></div>
    ...
    <form method="POST" action="<?= $sport ? '/admin/sports/update/' . $sport['id'] : '/admin/sports/store' ?>">
        <?= csrf_field() ?>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nom" class="form-input" value="<?= old('nom', $sport['nom'] ?? '') ?>" placeholder="Ex: Course, Natation, Football..." required>
                </div>

                <div class="form-group">
                    <label class="form-label">Calories par heure *</label>
                    <input type="number" name="calories_par_heure" class="form-input" step="0.1" value="<?= old('calories_par_heure', $sport['calories_par_heure'] ?? '') ?>" placeholder="Ex: 600" required>
                </div>

                <div class="btn-group">
                    <button type="submit" class="btn btn-primary"><?= $sport ? 'Mettre à jour' : 'Créer' ?></button>
                    <a href="/admin/sports" class="btn btn-secondary">Annuler</a>
                </div>
